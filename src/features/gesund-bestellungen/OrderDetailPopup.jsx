...
import useGesundOrderDetail from './useGesundOrderDetail'
...
export default function OrderDetailPopup({ order, theme, onClose, getViewUrl, printFile }) {
  const { detail, loading: detailLoading, loadDetail, clearDetail } = useGesundOrderDetail()
  ...
  // Beim Öffnen: Detail nachladen
  useEffect(() => {
    loadDetail(order.id)
    return () => clearDetail()
  }, [order.id, loadDetail, clearDetail])

  const chatMessages = detail ? (detail.chat_messages || []).filter(m => m.type === 'CHAT') : []
  const erezeptCodes = detail ? (detail.erezept_codes || []) : []
  const prescriptionImages = useMemo(() => detail?.prescription_images || [], [detail])
  ...
  // Prescription images signed URLs
  useEffect(() => {
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {detailLoading ? (
            <div className="flex items-center justify-center py-12">
              <ArrowClockwise size={24} className={`animate-spin ${theme.textMuted}`} />
            </div>
          ) : detail ? (
            <>
              {/* Kontaktdaten */}
              <div className="space-y-2">
                {detail.email && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm ${theme.text}`}>{detail.email}</span>
                  </div>
                )}
                {detail.phone && (
                  <div className="flex items-center gap-2">
                    <Phone size={16} className={theme.textMuted} />
                    <span className={`text-sm ${theme.text}`}>{detail.phone}</span>
                  </div>
                )}
                {detail.address && (
                  <div className="flex items-center gap-2">
                    <MapPin size={16} className={theme.textMuted} />
                    <span className={`text-sm ${theme.text}`}>{detail.address}</span>
                  </div>
                )}
              </div>
                ...
                {detail.dispatch_type && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-medium ${theme.text}`}>{DISPATCH_LABELS[detail.dispatch_type] || detail.dispatch_type}</p>
                    </div>
                  </div>
                )}
                {detail.payment_method && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-medium ${theme.text}`}>{detail.payment_method}</p>
                    </div>
                  </div>
                )}
                {order.partner_name && (
