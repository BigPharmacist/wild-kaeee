...
/**
 * Shared week block used by both WeekGrid (single week) and MonthTable (per KW).
 *
 * @param {object}   props
 * @param {object}   props.theme
 * @param {Array}    props.days       – exactly 5 entries (Mon–Fri); gaps use { empty: true, dayName }
 * @param {Array}    props.schedules  – full schedule entries (filtered to relevant date range by caller)
 * @param {Array}    props.shifts     – all shift definitions
 * @param {Array}    props.profiles   – staff profiles
 * @param {Function} props.onCellClick(staffId, dateStr)
 */
export function WeekBlock({ theme, days, schedules, shifts, profiles, onCellClick }) {
  // Profile lookup
  const profileMap = {}
  profiles.forEach(p => {
